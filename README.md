...
For real DeepSeek evidence-generation pilots, train `llm_with_road_learning_balanced_threshold` with a more exploratory actor and keep learned-only test:
...
Set `DEEPSEEK_API_KEY` before running real pilots. The `acting_group` field records whether a row was generated by the train evidence policy or by the tested learned policy.
